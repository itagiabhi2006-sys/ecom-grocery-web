...
import {
  ShoppingCart, MapPin, Trash2, Check, Package, Edit2, Plus, ArrowRight,
} from "lucide-react";
...
export default function Checkout() {
  ...
  const handleSelectAddress = (addr) => {
    ...
    setForm({ name: addr.fullName, phone: addr.phone, address: addr.address, city: addr.city, pincode: addr.pincode });
  };
  ...
  const handleUpdateAddress = async (id) => {
    try {
      await api.post(`/user-address-update/${id}`, {
        fullName: form.name, phone: form.phone, address: form.address, city: form.city, pincode: form.pincode,
      });
  ...
  const handleSaveNewAddress = async () => {
    ...
    try {
      const res = await api.post("/add-address", {
        fullName: form.name, userId: Number(user.id), phone: form.phone,
  ...
  const proceedToPayment = () => {
    ...
    navigate("/payment", {
      state: { addressId: selectedAddress.id, offerApplied, offerAmount },
    });
  };
  ...
  if (cart.length === 0) {
    return (
      <div style={{ background: "#f9fafb", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ background: "#fff", borderRadius: 12, border: "1px solid #f0f0f0", padding: "48px 32px", textAlign: "center", maxWidth: 400 }}>
          <ShoppingCart size={48} color="#9ca3af" style={{ margin: "0 auto 16px" }} />
          <h2 style={{ fontSize: 20, fontWeight: 700, color: "#111827", marginBottom: 8 }}>Your cart is empty</h2>
          <p style={{ color: "#6b7280", fontSize: 13, marginBottom: 20 }}>Add some items to get started</p>
          <button
            onClick={() => navigate("/")}
            style={{ background: "#1e3a8a", color: "#fff", border: "none", borderRadius: 8, padding: "8px 24px", fontSize: 13, fontWeight: 600, cursor: "pointer" }}
    ...
    <div style={{ background: "#f9fafb", minHeight: "100vh" }}>
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "28px 24px" }}>
        ...
        <div style={{ marginBottom: 28 }}>
          ...
          <p style={{ color: "#6b7280", fontSize: 13, margin: "6px 0 0" }}>Complete your order in a few simple steps</p>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 360px", gap: 28, alignItems: "start" }}>
          ...
          {/* RIGHT - Order Summary with Better Spacing */}
          <div style={{ position: "sticky", top: 24 }}>
            <div style={{ background: "#fff", borderRadius: 12, border: "1px solid #f0f0f0", padding: "20px" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16 }}>
                <Package size={18} color="#1e3a8a" />
                <h2 style={{ fontSize: 14, fontWeight: 700, color: "#111827", margin: 0 }}>Order Summary</h2>
                <span style={{ marginLeft: "auto", fontSize: 11, color: "#6b7280" }}>{cart.length} items</span>
              </div>

              <div style={{ maxHeight: 280, overflowY: "auto", marginBottom: 16 }}>
                {cart.map((item, i) => {
                  const unitPrice = resolvePrice(item.products);
                  const hasDiscount = item.products?.discountFinalPrice != null && item.products?.discountFinalPrice !== 0;
                  return (
                    <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 0", borderBottom: i < cart.length - 1 ? "1px solid #f0f0f0" : "none" }}>
                      <div style={{ flex: 1 }}>
                        <p style={{ fontSize: 12, fontWeight: 500, color: "#111827", margin: 0, lineHeight: 1.3 }}>
                          {item.products.title.length > 30 ? item.products.title.substring(0, 27) + "..." : item.products.title}
                        </p>
                        <p style={{ fontSize: 10, color: "#6b7280", margin: "4px 0 0" }}>
                          Qty: {item.quantity} × ₹{unitPrice.toFixed(0)}
                          {hasDiscount && <span style={{ marginLeft: 6, textDecoration: "line-through", color: "#9ca3af" }}>₹{item.products?.price}</span>}
                        </p>
                      </div>
                      <span style={{ fontSize: 13, fontWeight: 600, color: "#1e3a8a" }}>₹{(item.quantity * unitPrice).toFixed(0)}</span>
                    </div>
                  );
                })}
              </div>
              ...
              <div style={{ borderTop: "1px solid #f0f0f0", paddingTop: 14 }}>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, marginBottom: 8 }}>
                  ...
                  <span style={{ fontWeight: 500 }}>₹{rawTotal.toFixed(2)}</span>
                </div>
                {offerApplied && offerAmount > 0 && (
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, marginBottom: 8, color: "#059669", fontWeight: 500 }}>
